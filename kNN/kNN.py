...
def getTrainingInfo(filename):
    fileStr = filename.split('.')[0]
    classNumStr = int(fileStr.split('_')[0])
    return filename , classNumStr
    pass
def handwritingClassTest():
    ...
    for i in range(m):
        fileNameStr ,classNumStr = getTrainingInfo(trainingFileList[i])
    ...
    for i in range(mTest):
        fileNameStr ,classNumStr = getTrainingInfo(testFileList[i])
    ...
    print("\nthe total number errors is:%d" % errorCount)
